...
while cv.waitKey(1) < 0:
	...
	for bbox in bboxes:
		...
		if age == '(8-12)':
			if flag:
				print("\n[INFO] Detected Age : {}, confidence = {:.3f}".format(age, agePreds[0].max()))
				print("\n[PLAY] Video Ad for Students")
				time.sleep(2)
				os.startfile(Path+'laptop.mp4')
				flag = False

		elif age == '(15-20)':
			if flag:
				print("\n[INFO] Detected Age : {}, confidence = {:.3f}".format(age, agePreds[0].max()))
				print("\n[PLAY] Video Ad for College")
				time.sleep(2)
				os.startfile(Path+'college.mp4')
				flag = False
			
		elif age == '(25-32)':
			if flag:
				print("\n[INFO] Detected Age : {}, confidence = {:.3f}".format(age, agePreds[0].max()))
				print("\n[PLAY] Video Ad for Job")
				time.sleep(2)
				os.startfile(Path+'job.mp4')
				flag = False

		elif age == '(38-43)':
			if flag:
				print("\n[INFO] Detected Age : {}, confidence = {:.3f}".format(age, agePreds[0].max()))
				print("\n[PLAY] Video Ad for Business")
				time.sleep(2)
				os.startfile(Path+'promotion.mp4')
				flag = False

		elif age == '(48-53)':
			if flag:
				print("\n[INFO] Detected Age : {}, confidence = {:.3f}".format(age, agePreds[0].max()))
				print("\n[PLAY] Video Ad for Loan")
				time.sleep(2)
				os.startfile(Path+'loan.mp4')
				flag = False
